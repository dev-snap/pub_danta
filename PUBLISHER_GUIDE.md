...
| `bet_request` | `bet_callback_url` | 베팅 시도 시 동기 호출 | **승인/거부 응답 필수** |
| `bet_confirmed` | `bet_callback_url` | 베팅 트랜잭션 커밋 직후 | 수신 확인 (비동기 알림) |
| `bet_canceled` | `bet_callback_url` | 라운드/베팅 취소로 환불 필요 | 수신 확인 + **매체 측 `media_amount` 환불 트리거** |

> 모든 콜백은 **1회만 발송**되며 리트라이가 없다. 누락 대비는 §5 REST API 폴링으로 보완할 것.
...
> 매체사는 이 시점에 자사 DB 에서 `media_amount` 만큼 포인트를 차감해야 한다.

### 4-7. `bet_confirmed` — 베팅 확정 알림 (비동기)

`bet_request` 승인 후 단타 서버가 트랜잭션을 커밋한 직후 발송. 채널은 `bet_callback_url`. 리트라이 없음 (1회 알림).
...
라운드/베팅이 무효 처리되어 매체사가 자사 DB 의 `media_amount` 를 환불해야 함을 알리는 콜백. 채널은 `bet_callback_url`. 리트라이 없음.
...
> race rejection 시 `bet_id` 가 없을 수 있다 (단타 서버에 베팅 INSERT 자체가 안 된 경우). 매체사는 `user_id` + `media_amount` 만으로 환불을 식별/처리하면 된다.
> 매체사는 이 콜백 수신 시 자사 DB 에서 해당 유저에게 `media_amount` 를 환불해야 한다 (이전에 `bet_request` 승인 시 차감된 양과 동일).
...
| 필드 | 타입 | 발송 콜백 | 설명 |
|---|---|---|---|
| `bet_id` | integer | confirmed / canceled (대부분) | 단타 서버 베팅 PK. `bet_canceled` 의 race rejection 케이스에서는 없을 수 있음 |
| `reason` | string | canceled | 취소 사유 (`"round_closed"` / `"engine_recovery_stale"` / `"inactive_game_cleanup"` 등) |
...
A. 10초 타임아웃으로 실패 처리. **모든 콜백은 1회만 발송**되며 재시도하지 않으니, 누락 대비는 §5 REST API 폴링으로 보완하세요.
