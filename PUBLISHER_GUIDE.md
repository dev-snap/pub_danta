...
| `media_payout_amount` | 매체 포인트 | `payout_amount ÷ point_rate` |
    ...
    "results": [
      {
        ...
        "media_bet_amount": 0.02, "media_payout_amount": 0.0, "settled_at": "..."
      },
      {
        "bet_id": 9912, "user_id": "u_42", "direction": "down",
        "bet_amount": 1000, "payout_amount": 1316, "bet_result": "win",
        "media_bet_amount": 0.02, "media_payout_amount": 0.02632, "settled_at": "..."
      }
    ]
  }
}
...
| 2026-05-18 | **Rename** — S3 정산/취소 JSON 의 `users[]` 항목, REST `GET /api/pub/games/:gameId/rounds/:roundId` 응답의 results 항목 필드 `media_payout` → **`media_payout_amount`** 로 변경. 명명 통일 (`bet_amount` / `payout_amount` / `media_bet_amount` / `media_payout_amount` 모두 `_amount` suffix). 동일 값(`payout_amount ÷ point_rate`). 출시 전 변경이라 dual emit 없이 hard rename. |
